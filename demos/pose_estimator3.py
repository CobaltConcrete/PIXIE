...
from pixielib.datasets.body_datasets import TestData, WebcamPreprocessor
...
def main(args):
    ...
    if webcam:
        preprocessor = WebcamPreprocessor(device=device)
        # ── 2a. Live webcam loop ───────────────────────────────────────────────
        print("[webcam] Starting live animation loop — press Ctrl-C to stop.")
        frame_idx = 0
        timing_log_path = os.path.join(savefolder, 'timing.txt')
        with open(timing_log_path, 'w') as timing_log:
            print(f"[timing] Logging per-frame timings to {timing_log_path}")
            try:
                while True:
                    t_total_start = time.perf_counter()

                    t0 = time.perf_counter()
                    pose_batch = preprocessor.process(webcam.read(), device, iscrop=False)
                    data       = {'body': pose_batch}
                    t_webcam = time.perf_counter() - t0
                    ...
                    t0 = time.perf_counter()
                    grid_visdict   = {'pose_ref_shape': pose_ref_shape}
                    ...
                    t_imwrite = time.perf_counter() - t0
                    ...
                    log_line = (
                        f"[frame {frame_idx:05}] "
                        f"webcam={t_webcam*1000:.1f}ms  "
                        f"encode={t_encode*1000:.1f}ms  "
                        f"decode={t_decode*1000:.1f}ms  "
                        f"reproject={t_reproject*1000:.1f}ms  "
                        f"render={t_render*1000:.1f}ms  "
                        f"grid={t_grid*1000:.1f}ms  "
                        f"imwrite={t_imwrite*1000:.1f}ms  "
                        f"stream={t_stream*1000:.1f}ms  "
                        f"TOTAL={t_total*1000:.1f}ms  ({1/t_total:.1f} fps)\n"
                    )
                    timing_log.write(log_line)
                    timing_log.flush()
